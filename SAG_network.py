...
class SAGNet(Network):
    def __init__(self, data, data_helper, for_training):
        ...
        if self.for_training:
            ...
            self.n_iter = data['config_dict']['TRAIN']['ITERATION_NUM']
    ...
    def setup_for_training(self):
        ...
        # Using three different optimizers to process the three modules in our framework
    ...
    def _setup_rnns(self):
        ...
        # gen_part_encoder_cell
        part_encoder_gru_cell = tf.contrib.rnn.GRUCell(self.rnn_state_dim, activation=tf.tanh)
        self.part_encoder_multi_cell = tf.contrib.rnn.MultiRNNCell([part_encoder_gru_cell] * self.rnn_cell_depth)
        self.part_encoder_multi_cell_state = self.part_encoder_multi_cell.zero_state(self.batch_size, tf.float32)

        # gen_part_decoder_cell
        part_decoder_gru_cell = tf.contrib.rnn.GRUCell(self.rnn_state_dim, activation=tf.tanh)
        self.part_decoder_multi_cell = tf.contrib.rnn.MultiRNNCell([part_decoder_gru_cell] * self.rnn_cell_depth)
        self.part_decoder_multi_cell_state = self.part_decoder_multi_cell.zero_state(self.batch_size, tf.float32)
    ...
    def _voxel_encoder(self, input_voxels, reuse=False):
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            ve_conv_block_2_3_out = self.conv_residual_block(ve_conv_block_2_2_out, 3, 64, init_weights,
                                                             've_conv_block_2_3', leaky_value=self.leaky_value, padding='SAME', bottle_neck=False)

            # second resnet block
            ve_conv_block_3_1_out = self.conv_residual_block(ve_conv_block_2_3_out, 3, 128, init_weights,
            ...
            ve_conv_block_3_3_out = self.conv_residual_block(ve_conv_block_3_2_out, 3, 128, init_weights,
                                                             've_conv_block_3_3', self.leaky_value, padding='SAME', bottle_neck=False)

            # third resnet block
            ve_conv_block_4_1_out = self.conv_residual_block(ve_conv_block_3_3_out, 3, 256, init_weights,
            ...
            ve_conv_block_4_4_out = self.conv_residual_block(ve_conv_block_4_3_out, 3, 256, init_weights,
                                                             've_conv_block_4_4', self.leaky_value, padding='SAME', bottle_neck=False)
            ve_conv_block_4_5_out = self.conv_residual_block(ve_conv_block_4_4_out, 3, 256, init_weights,
                                                             've_conv_block_4_5', self.leaky_value, padding='SAME', bottle_neck=False)

            ve_conv_out = self.conv3d(ve_conv_block_4_5_out, 2, 512, 've_conv_out', init_weights, strides=same_strides, init_biases=init_biases,
                                      leaky_value=self.leaky_value, relu=True, batch_norm=True, padding='SAME')
            return ve_conv_out
    ...
    def _voxel_decoder(self, input_voxel_features, reuse=False):
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            vd_deconv_3_2_out = self.deconv_residual_block(vd_deconv_3_1_out, 3, 128, (batch_size, 8, 8, 8, 128),
                                                           ...
                                                           padding='SAME', bottle_neck=False)
            vd_deconv_3_3_out = self.deconv_residual_block(vd_deconv_3_2_out, 3, 128, (batch_size, 8, 8, 8, 128),
                                                           init_weights, 'vd_deconv_3_3_block', self.leaky_value,
                                                           padding='SAME', bottle_neck=False)
            # third deconv block
            vd_deconv_4_1_out = self.deconv_residual_block(vd_deconv_3_3_out, 3, 64, (batch_size, 16, 16, 16, 64),
            ...
            vd_deconv_4_2_out = self.deconv_residual_block(vd_deconv_4_1_out, 3, 64, (batch_size, 16, 16, 16, 64),
                                                           ...
                                                           padding='SAME', bottle_neck=False)
            vd_deconv_4_3_out = self.deconv_residual_block(vd_deconv_4_2_out, 3, 64, (batch_size, 16, 16, 16, 64),
                                                           init_weights, 'vd_deconv_4_3_block', self.leaky_value,
                                                           padding='SAME', bottle_neck=False)
            vd_deconv_4_4_out = self.deconv_residual_block(vd_deconv_4_3_out, 3, 64, (batch_size, 16, 16, 16, 64),
                                                           init_weights, 'vd_deconv_4_4_block', self.leaky_value,
                                                           padding='SAME',  bottle_neck=False)
            vd_deconv_4_5_out = self.deconv_residual_block(vd_deconv_4_4_out, 3, 64, (batch_size, 16, 16, 16, 64),
                                                           init_weights, 'vd_deconv_4_5_block', self.leaky_value,
                                                           padding='SAME',  bottle_neck=False)

            vd_deconv_out = self.deconv3d(vd_deconv_4_5_out, 3, 1, 'vd_deconv_5_1', (batch_size, 32, 32, 32, 1),
    ...
    def _vert_rnn_forward(self, vert_in, reuse=False):
        with tf.variable_scope('vert_rnn'):
            ...
            # since we need merge the vert information and edge information, so we should hold the rnn cell states
    ...
    def _learn_representation_for_graph(self, input_vert_feature, input_edge_feature, part_visible_masks, gaussian_noise,
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            (self.feed(obj_embedding)
                 .fc(self.embedding_size, relu=False, name='graph_vector_fc', reuse=reuse, trainable=phase_train)
            ...
            # sigma_name = layer_name + '/graph_embedding_sigma'
    ...
    def get_batch_info(self, sess, inputs_data):
        ...
        voxel_ops, bbox_ops = self.pred_voxel_and_bbox()

        d_voxels = sess.run(voxel_ops, feed_dict=input_feed)
        d_bboxs = sess.run(bbox_ops, feed_dict=input_feed)
